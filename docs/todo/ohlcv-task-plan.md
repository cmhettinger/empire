...
Before making changes, read AGENTS.md, docs/todo/ohlcv-plan.md, and the full
docs/todo/ohlcv-task-plan.md. Inspect the current repository state and the Done:
notes for completed prerequisite tasks; do not assume the plan is newer than the
live code.
...
`Done:` note and the current live repository state.
