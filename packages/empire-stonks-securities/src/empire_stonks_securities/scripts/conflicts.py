"""Generate the Phase 2A stonks securities conflict report."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    with EmpireDatabase.connect_from_env() as connection:
        ...
        if args.output == "console":
            ...
            path = default_conflict_report_path(
                temp_dir=args.temp_dir,
                generated_at=generated_at,
            )
            write_conflict_report_to_file(report, path)
            print(f"conflict_report_path: {path}")
        else:
            path = write_conflict_report_to_file(report, args.output)
            print(f"conflict_report_path: {path}")
        ...
        if args.object_store:
            ...
            print(f"conflict_report_object_id: {stored.object_id}")


def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Generate the stonks securities conflict report."
    )
    parser.add_argument(
        "--output",
        default="console",
