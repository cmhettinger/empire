"""Publish a local stonks securities config file to the Empire object store."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    StonksSecuritiesConfig.from_yaml(data.decode("utf-8"))
...
def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Publish a local stonks securities config to object store."
    )
    parser.add_argument(
        "config_file",
        nargs="?",
        help="Local stonks securities YAML config file.",
    )
    parser.add_argument(
        "--logical-name",
    ...
    parser.add_argument(
        "--storage-key",
        help="Object key prefix. Defaults to stonks-securities.",
    )
